...
type TranscriptContextValue = {
  ...
  restoreTranscript: (items: Array<{ role: "user" | "assistant"; content: string; timestamp?: number }>) => void;
};
...
export const TranscriptProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const restoreTranscript: TranscriptContextValue["restoreTranscript"] = (items) => {
    if (transcriptItems.length > 0) {
      // Don't restore if transcript already has items (session in progress)
      console.log('SKIPPING RESTORE: Transcript already has items');
      return;
    }

    const restoredItems: TranscriptItem[] = items.map((item, idx) => ({
      itemId: `restored-${idx}-${item.timestamp || Date.now()}`,
      type: "MESSAGE",
      role: item.role,
      title: item.content,
      expanded: false,
      timestamp: newTimestampPretty(),
      createdAtMs: item.timestamp || Date.now() - (items.length - idx) * 1000, // Space them out
      status: "DONE",
      isHidden: false,
    }));

    console.log('RESTORING TRANSCRIPT:', { itemCount: restoredItems.length });
    setTranscriptItems(restoredItems);
  };
