...
export function useHandleSessionHistory() {
  ...
  function handleHistoryAdded(item: any) {
    ...
    if (itemId && role) {
      ...
      if (guardrailMessage) {
        ...
        // Log AI responses to see if it's using memories
  ...
  function handleTranscriptionCompleted(item: any) {
    ...
    if (itemId) {
      ...
      // Log final AI response to see if it's using memories
      if (transcriptItem?.role === 'assistant' && finalTranscript && finalTranscript !== '[inaudible]') {
        console.log('AI FINAL RESPONSE:', finalTranscript);
      }
