/**
 * Chat API Endpoint - Main System
 * 
 * Uses the same logic as /api/chat-test
 * Unified system for both voice and text chat
 * 
 * Features:
 * - Memory extraction and storage
 * - "React once, then ask once" structure
 * - Controlled memory usage (sparingly)
 * - Experience memories from Redis
 */
 ...
 * Works for both voice and text chat
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // FAST PATH: Handle greetings and farewells immediately (no Redis, no memory operations)
